  ...
  try {
    ...
    for (const auto outrec : outrec_list_) {
	  ...
	  else
	  {
		  ...
		  if (op->pt == outrec->pts->next->pt) cnt--;
		  if (cnt < 3) continue;
	  }
	  ...
	  if (outrec->owner)
	  {
		  int cnt2 = PointCount(outrec->owner->pts);
		  std::cout << "i: " << outrec->idx << ", Owner: " << outrec->owner->idx << ", Owner cnt" << cnt2 << ", cnt: " << cnt << std::endl;
		  if (cnt == 0 && outrec->owner->owner)
		  {
			  int cnt3 = PointCount(outrec->owner->owner->pts);
			  std::cout << "i: " << outrec->idx << ", Owner of Owner: " << outrec->owner->owner->idx << ", Owner cnt" << cnt3 << ", cnt: " << cnt << std::endl;
		  }
	  }
	  else
